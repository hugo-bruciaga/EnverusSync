...
def get_buckets_production(qry_filter):
    ...
    sql = (f" with partitions as ("
           ...
           f"where {qry_filter}"
           f" and producingmonth between '01-jan-1960' and '01-jan-1970'"  # Hugo
           f")"
    ...
    '''
    sql = ("select 1 Bucket, '1916-08-01' min, '1960-06-01' max union "
           "select 2 Bucket, '1960-06-01' min, '1965-03-01' max union "
           "select 3 Bucket, '1965-03-01' min, '1968-11-01' max union "
           "select 4 Bucket, '1972-02-01' min, '1975-01-01' max union "
           "select 5 Bucket, '1975-01-01' min, '1977-07-01' max"
           )
    '''
...
def s_thread(table_name, qry_filter, bucket0, bucket1, bucket2, stored_procedure):
    log_file('Processing Bucket (' + str(bucket0) + '):', str(bucket1), 'thru:', str(bucket2))
    src_cn = db_connect(env_srv, env_db, env_user, env_pw)
    dst_cn = db_connect(ar_srv, ar_db)
    dst_cn.autocommit = True  # Added to prevent: was deadlocked on lock resources with another process and has been
    ...
    sql = (f"SELECT * FROM {schema_name}.{table_name}\n "
           ...
           f"   AND producingmonth >= '{bucket1}'\n"
           f"   AND producingmonth < '{bucket2}'\n"
           )
    ...
    if data != 'null':
        log_file(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), bucket0, 'Time Elapsed Reading:',
        ...
        log_file(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), bucket0, 'Time Elapsed Writing:',
...
def sp_save(batch_size, table_name, unique_cols, stored_procedure, qry_filter):
    ...
    if table_name == 'Foundations_Production':
        ...
        for bucket in buckets:
            if bucket[0] in range(1, 5):
                t = threading.Thread(target=s_thread,
                                     args=(table_name, qry_filter, bucket[0], bucket[1], bucket[2], stored_procedure))
